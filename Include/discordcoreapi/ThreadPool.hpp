...
namespace DiscordCoreAPI {
	...
	class DiscordCoreAPI_Dll ThreadPool {
	  public:
		...
		static std::string storeThread(TimeElapsedHandler<ArgTypes...> timeElapsedHandler,
			int32_t timeInterval, bool repeated, ArgTypes... args) {
			std::string threadId =
			...
			ThreadPool::threads[threadId] = std::jthread([=](std::stop_token stopToken) {
				...
				while (true) {
					...
					std::this_thread::sleep_for(std::chrono::milliseconds{ static_cast<int32_t>(
						std::ceil(static_cast<float>(timeInterval) * thePercentage)) });
		...
		static std::string storeThread(TimeElapsedHandlerTwo timeElapsedHandler,
			int32_t timeInterval, bool repeated) {
			std::string threadId =
				std::to_string(std::chrono::duration_cast<std::chrono::nanoseconds>(
					std::chrono::system_clock::now().time_since_epoch())
								   .count());
		...
		template<typename... ArgTypes>
		static void executeFunctionAfterTimePeriod(
			TimeElapsedHandler<ArgTypes...> timeElapsedHandler, int32_t timeInterval,
