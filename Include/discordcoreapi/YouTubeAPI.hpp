...
namespace DiscordCoreInternal {
	...
	class DiscordCoreAPI_Dll YouTubeRequestBuilder {
	  ...
	  protected:
		...
		DiscordCoreAPI::YouTubeFormat decipherFormat(DiscordCoreAPI::YouTubeFormat format, std::string const& html5playerFile);

		std::string decipher(std::vector<std::string> tokens, std::string const& cipherSignature);

		std::vector<std::string> extractActions(std::string const& html5PlayerPageBody);

		std::vector<char> reverseString(std::vector<char> stringToReverse);

		std::string between(std::string const& body, std::string const& left, std::string const& right);

		std::vector<std::string> getTokens(std::string const& html5PlayerFile);

		std::vector<char> splitString(std::string const& stringToSplit);
	...
	class DiscordCoreAPI_Dll YouTubeAPI {
	  public:
		YouTubeAPI(std::string const& guildId, HttpClient* httpClient);
