...
namespace DiscordCoreAPI {
	...
	struct JsonValue {
			...
			for (auto& value: theValues) {
				switch (value.theType) {
					case ObjectType::Array_Start: {
						...
						this->haveWeStarted.push_back(false);
						this->areWeCountingArrayLength = true;
						break;
					}
					case ObjectType::Array_End: {
						this->theValue += "]";
					...
					case ObjectType::Struct_Start: {
						if (this->areWeCountingArrayLength) {
		...
		std::string parseJsonToEtf();

	  protected:
		std::vector<JsonScalarObject<>> theValues{};
		std::vector<bool> haveWeStarted{ false };
		int32_t currentArrayLength{ 0 };
		bool areWeCountingArrayLength{};
		std::string theValue{};
		size_t offSet{};
		size_t size{};
		...
		std::string parseJsonToEtf() {
			...
			this->size = 0;
			this->appendVersion();
			this->singleValueJsonToETF();
			return this->theValue;
		}

		void singleValueJsonToETF() {
			for (auto& value: this->theValues) {
				if (value.theType == ObjectType::Array_Start) {
					uint32_t length = {};
					if (length == 0) {
						this->appendNilExt();
					} else {
						if (length > std::numeric_limits<uint32_t>::max() - 1) {
							throw std::runtime_error{ "singleValueJsonToETF() Error: List too large for ETF.\n\n" };
						}
					}
					...
					this->appendNilExt();
					if (value.theType == ObjectType::Struct_Start) {
						uint32_t length = static_cast<uint32_t>(jsonData.size());
						if (length > std::numeric_limits<uint32_t>::max() - 1) {
							throw std::runtime_error{ "singleValueJsonToETF() Error: Map too large for ETF.\n\n" };
						}
					...
					} else if (value.theType == ObjectType::Integer_16_Bit || value.theType == ObjectType::Integer_8_Bit || value.theType == ObjectType::Integer_32_Bit ||
						value.theType == ObjectType::Integer_64_Bit || value.theType == ObjectType::Unsigned_Integer_8_Bit ||
						value.theType == ObjectType::Unsigned_Integer_16_Bit || value.theType == ObjectType::Unsigned_Integer_32_Bit ||
						value.theType == ObjectType::Unsigned_Integer_64_Bit) {
						uint64_t numberOld = stoull(value.theValue);
						if (numberOld <= 127) {
							uint8_t number = stoull(value.theValue);
							...
							uint64_t number = stoull(value.theValue);
							this->appendUnsignedLongLong(number);
						} else {
							uint32_t number = stoull(value.theValue);
							this->appendIntegerExt(number);
						}
					} else if (value.theType == ObjectType::Boolean) {
						if (stoull(value.theValue)) {
							this->appendTrue();
						} else {
							this->appendFalse();
						}
					} else if (value.theType == ObjectType::String) {
						std::string newString = value.theValue;
					...
					} else if (value.theType == ObjectType::Float_32_Bit || value.theType == ObjectType::Float_64_Bit) {
						double newValue = jsonData.get<double>();
						this->appendNewFloatExt(newValue);
					} else if (value.theType == ObjectType::Unset) {
						this->appendNil();
					}
				}
			}
			...
			void writeToBuffer(const std::string& bytes) {
				this->theValue.insert(this->theValue.end(), bytes.begin(), bytes.end());
				this->offSet += bytes.size();
			}

			void appendVersion() {
				std::string bufferNew{};
				bufferNew.push_back(static_cast<char>(131));
				this->writeToBuffer(bufferNew);
			}

			void appendSmallIntegerExt(uint8_t value) {
				std::string bufferNew{ static_cast<uint8_t>(DiscordCoreInternal::ETFTokenType::Small_Integer_Ext), static_cast<char>(value) };
				this->writeToBuffer(bufferNew);
			}

			void appendIntegerExt(uint32_t value) {
				std::string bufferNew{ static_cast<uint8_t>(DiscordCoreInternal::ETFTokenType::Integer_Ext) };
			...
			void appendNilExt() {
				std::string bufferNew{ static_cast<uint8_t>(DiscordCoreInternal::ETFTokenType::Nil_Ext) };
				this->writeToBuffer(bufferNew);
			}

			void appendNil() {
				std::string bufferNew{ static_cast<uint8_t>(DiscordCoreInternal::ETFTokenType::Small_Atom_Ext), 3, static_cast<uint8_t>('n'), static_cast<uint8_t>('i'),
					static_cast<uint8_t>('l') };
				this->writeToBuffer(bufferNew);
			}

			void appendFalse() {
				std::string bufferNew{ static_cast<uint8_t>(DiscordCoreInternal::ETFTokenType::Small_Atom_Ext), 5, static_cast<uint8_t>('f'), static_cast<uint8_t>('a'),
					static_cast<uint8_t>('l'),
			...
			void appendTrue() {
				std::string bufferNew{ static_cast<uint8_t>(DiscordCoreInternal::ETFTokenType::Small_Atom_Ext), 4, static_cast<uint8_t>('t'), static_cast<uint8_t>('r'),
					static_cast<uint8_t>('u'),
					static_cast<uint8_t>('e') };
				this->writeToBuffer(bufferNew);
			}

			void appendUnsignedLongLong(uint64_t value) {
				std::string bufferNew{};
				bufferNew.resize(static_cast<uint64_t>(1) + 2 + sizeof(uint64_t));
				bufferNew[0] = static_cast<uint8_t>(DiscordCoreInternal::ETFTokenType::Small_Big_Ext);
			...
			void appendBinaryExt(const std::string& bytes, uint32_t sizeNew) {
				std::string bufferNew{ static_cast<uint8_t>(DiscordCoreInternal::ETFTokenType::Binary_Ext) };
				DiscordCoreAPI::storeBits(bufferNew, sizeNew);
				this->writeToBuffer(bufferNew);
				this->writeToBuffer(bytes);
			}

			void appendListHeader(uint32_t sizeNew) {
				std::string bufferNew{ static_cast<uint8_t>(DiscordCoreInternal::ETFTokenType::List_Ext) };
				DiscordCoreAPI::storeBits(bufferNew, sizeNew);
				this->writeToBuffer(bufferNew);
			}

			void appendMapHeader(uint32_t sizeNew) {
				std::string bufferNew{ static_cast<uint8_t>(DiscordCoreInternal::ETFTokenType::Map_Ext) };
				DiscordCoreAPI::storeBits(bufferNew, sizeNew);
				this->writeToBuffer(bufferNew);
			}
	};
