...
namespace DiscordCoreAPI {
	...
	DiscordCoreAPI_Dll std::string utf8MakeValid(std::string inputString);
	...
	/// User's flags. \brief User's flags.
	...
	enum class ChannelFlags : int8_t {
		NSFW = 1 << 0,///< NSFW Channel?
	...
	class DiscordCoreAPI_Dll ChannelData : public DiscordEntity {
	  public:
		...
		void setPinned(bool enabled) {
			if (enabled) {
				this->flags |= static_cast<uint8_t>(ChannelFlags::Pinned);
			} else {
				this->flags &= ~static_cast<uint8_t>(ChannelFlags::Pinned);
			}
		}
		...
		bool getPinned() {
			return this->flags & static_cast<uint8_t>(ChannelFlags::Pinned);
		}
	...
	class DiscordCoreAPI_Dll GuildData : public DiscordEntity {
	  public:
		...
		std::vector<std::string> roles{};///< Array of Guild rol
	...
	class DiscordCoreAPI_Dll RespondToInputEventData {
	  public:
		...
		/// \returns RespondToInputEventData& A reference to this data structure.
		RespondToInputEventData& addModal(std::string topTitleNew, std::string topCustomIdNew, std::string titleNew, std::string customIdNew, bool required, int32_t minLength,
