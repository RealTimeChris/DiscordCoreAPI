...
namespace DiscordCoreAPI {
	...
	User::User(UserData dataNew) {
		...
		if (dataNew.mfaEnabled) {
			this->flagsEnum |= static_cast<int8_t>(UserFlags::MFAEnabled);
		}
		if (dataNew.verified) {
			this->flagsEnum |= static_cast<int8_t>(UserFlags::Verified);
		}
		this->createdAt = dataNew.createdAt;
		this->userName = dataNew.userName;
		this->locale = dataNew.locale;
		if (dataNew.system) {
			this->flagsEnum |= static_cast<int8_t>(UserFlags::System);
		}
		if (dataNew.bot) {
			this->flagsEnum |= static_cast<int8_t>(UserFlags::Bot);
		}
	...
	BotUser::BotUser(User dataPackage, DiscordCoreInternal::BaseSocketAgent* baseSocketAgentNew) :User(dataPackage) {
