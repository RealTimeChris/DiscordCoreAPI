...
namespace DiscordCoreAPI {
    ...
    std::string getTimeAndDate() {
        ...
        auto at = GetDynamicTimeZoneInformation(&timeZoneInfo);
        localtime_s(&time, &now);
        if (static_cast<int32_t>(at) == 1) {
    ...
    std::vector<ApplicationCommandInteractionDataOption> DiscordCoreAPI::convertAppCommandInteractionDataOptions(std::vector<ApplicationCommandInteractionDataOption> originalOptions) {
    ...
    std::string convertTimeInMsToDateTimeString(int64_t timeInMs, TimeFormat timeFormat) {
        ...
        if (error) {
            std::cout << "Invalid argument to _localtime64_s." << std::endl;
        }
        char charArray[48];
        std::string timeStamp{};
        switch (timeFormat) {
        case TimeFormat::LongDate: {
            strftime(charArray, 48, "%d %B %G", &timeInfo);
        ...
        case TimeFormat::LongDateTime: {
            strftime(charArray, 25, "%a %b %d %Y %X", &timeInfo);
        ...
        case TimeFormat::LongTime: {
            strftime(charArray, 48, "%ObjectType", &timeInfo);
            for (int32_t x = 0; x < 48; x += 1) {
                timeStamp.push_back(charArray[x]);
            }
        ...
        case TimeFormat::ShortDate: {
            strftime(charArray, 48, "%d/%m/%g", &timeInfo);
            for (int32_t x = 0; x < 48; x += 1) {
                timeStamp.push_back(charArray[x]);
            }
            timeStamp = timeStamp.substr(0, timeStamp.find_last_of("/") + 3);
        ...
        case TimeFormat::ShortDateTime: {
            strftime(charArray, 48, "%d %B %G %R", &timeInfo);
            for (int32_t x = 0; x < 48; x += 1) {
                timeStamp.push_back(charArray[x]);
            }
            break;
        ...
        case TimeFormat::ShortTime: {
            strftime(charArray, 48, "%R", &timeInfo);
            for (int32_t x = 0; x < 48; x += 1) {
                timeStamp.push_back(charArray[x]);
            }
