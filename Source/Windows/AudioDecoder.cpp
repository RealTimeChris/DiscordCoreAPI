...
namespace DiscordCoreAPI {
    ...
    TSUnboundedMessageBlock<std::vector<uint8_t>>& AudioDecoder::getInputBuffer() {
        return this->inputDataBuffer;
    }

    void AudioDecoder::updateBufferRefreshTime(int32_t newRefreshTime) {
        this->refreshTimeForBuffer = newRefreshTime;
    }

    bool AudioDecoder::getFrame(RawFrameData& dataPackage) {
        if (!this->areWeQuitting) {
            RawFrameData rawFrame;
            if (this->outDataBuffer.tryReceive(rawFrame)) {
                if (rawFrame.sampleCount != -1) {
                    dataPackage = std::move(rawFrame);
                    return true;
                }
            }
        }
        ...
        try {
            if (!this->haveWeBooted) {
                ...
                if (fileStreamBuffer == nullptr) {
                    ...
                    throw std::exception("Failed to allocate filestreambuffer.\n\n");
                }
                ...
                if (this->ioContext == nullptr) {
                    this->haveWeFailedBool.store(true, std::memory_order_release);
                    throw std::exception("Failed to allocate AVIOContext.\n\n");
                }
                ...
                if (!this->formatContext) {
                    this->haveWeFailedBool.store(true, std::memory_order_release);
                    throw std::exception("Could not allocate the format context.\n\n");
                }
                ...
                if (avformat_open_input(*this->formatContext, "memory", nullptr, nullptr) < 0) {
                    this->haveWeFailedBool.store(true, std::memory_order_release);
                    throw std::exception("Error opening AVFormatContext.\n\n");
                }
                *this->formatContext.getBoolPtr() = true;
                ...
                if (ret < 0) {
                    ...
                    this->haveWeFailedBool.store(true, std::memory_order_release);
                    throw std::exception(newString.c_str());
                }
                    ...
                    if (!this->audioStream) {
                        this->haveWeFailedBool.store(true, std::memory_order_release);
                        throw std::exception("Could not find an audio stream.\n\n");
                    }
            ...
            if (this->currentBuffer.size() > 0) {
                ...
                if (!this->frame) {
                    this->haveWeFailedBool.store(true, std::memory_order_release);
                    throw std::exception("Error: Could not allocate frame\n\n");
                }

                this->newFrame = av_frame_alloc();
                if (!this->newFrame) {
                    this->haveWeFailedBool.store(true, std::memory_order_release);
                    throw std::exception("Error: Could not allocate new-frame\n\n");
                }
                    ...
                    if (this->packet->stream_index == this->audioStreamIndex) {
                        ...
                        if (ret >= 0) {
                            ...
                            if (ret < 0 || newFrame->nb_samples == 0) {
                                this->haveWeFailedBool.store(true, std::memory_order_release);
                                throw std::exception("Return value is less than zero!\n\n");
                            }
                        }
                        else {
                            break;
