...
void myPurecallHandler() {
	std::cout << "Purecall Handler Called, Current Thread: " << std::this_thread::get_id() << std::endl;
}
...
namespace DiscordCoreAPI {
	...
	DiscordCoreClient::DiscordCoreClient(std::string botTokenNew, std::string commandPrefixNew, std::vector<RepeatedFunctionData> functionsToExecuteNew,  CacheOptions cacheOptionsNew) {
		...
		for (auto& value : this->functionsToExecute) {
			...
			ThreadPoolTimer::ThreadPoolTimer::ThreadPoolTimer::executeFunctionAfterTimePeriod(value.function, static_cast<int32_t>(value.intervalInMs), value.repeated, thePtr);
		}
	}

	DiscordCoreClient::DiscordCoreClient(nullptr_t nullPtr) {}
	...
	std::string DiscordCoreClient::getGateWayBot() {
		try {
			...
			auto result = this->httpClient->submitWorkloadAndGetResult<GatewayBotData>(workload);
