	...
	bool DiscordCoreClient::instantiateWebSockets() {
		...
		for (int32_t x = 0; x < shardGroupCount; x += 1) {
			for (int32_t y = 0; y < shardsPerGroup; y += 1) {
				...
				auto thePtr = std::make_unique<DiscordCoreInternal::BaseSocketAgent>(new DiscordCoreInternal::BaseSocketAgent{ this->botToken, "127.0.0.1", &this->eventManager,
					this, &this->commandController, &Globals::doWeQuit, this->loggingOptions.logWebSocketSuccessMessages, this->loggingOptions.logWebSocketErrorMessages,
