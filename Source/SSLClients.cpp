...
namespace DiscordCoreInternal {
	...
	bool HttpSSLClient::connect(std::string& baseUrl, std::string portNew) noexcept {
		try {
			...
			if (baseUrl.find(".com") != std::string::npos) {
				stringNew = baseUrl.substr(baseUrl.find("https://") + std::string("https://").size(), baseUrl.find(".com") + std::string(".com").size() - std::string("https://").size());
			}
			else if (baseUrl.find(".org") != std::string::npos) {
				stringNew = baseUrl.substr(baseUrl.find("https://") + std::string("https://").size(), baseUrl.find(".org") + std::string(".org").size() - std::string("https://").size());
			}
			...
			if (std::unique_ptr<X509, X509Deleter> cert{ SSL_get_peer_certificate(this->ssl) }; cert == nullptr) {
		...
		maxBufferSize(maxBufferSizeNew)
	{
		try {
			...
			if (auto returnValue = setsockopt(this->theSocket, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char*>(&optionValue), sizeof(optionValue)); returnValue == SOCKET_ERROR) {
