...
namespace DiscordCoreInternal {
	...
	void BaseSocketAgent::onClosed(int32_t theIndex) noexcept {
		...
		if (this->theClients.contains(theIndex) && this->maxReconnectTries > this->theClients[theIndex]->currentRecursionDepth) {
			...
			theData.currentReconnectionDepth = this->theClients[theIndex]->currentRecursionDepth;
			theData.sessionId = this->theClients[theIndex]->sessionId;
			theData.areWeResuming = this->theClients[theIndex]->areWeResuming;
		...
		} else if (this->maxReconnectTries <= this->theClients[theIndex]->currentRecursionDepth) {
			...
			case WebSocketOpCode::Op_Pong: {
				...
				if (length01 == webSocketPayloadLengthMagicLarge) {
					...
					for (int64_t x = 2, shift = 56; x < 10; ++x, shift -= 8) {
						...
						theShard.messageLength |= static_cast<uint64_t>((lengthNew & 0xff) << static_cast<uint64_t>(shift));
					}
