			...
			try {
				...
				if (payload.contains("t") && !payload["t"].is_null()) {
					...
					if (payload["t"] == "READY") {
						...
						theUser.parseObject(&payload["d"]["user"]);
								...
								switch (EventConverter{ payload["t"].get<std::string>() }) {
									...
									case 25: {
										if (this->discordCoreClient->configManager.doWeCacheGuildMembers()) {
											...
											auto theGuildMemberNew = std::make_unique<DiscordCoreAPI::GuildMemberData>();
											theGuildMemberNew->parseObject(&payload["d"]);
