	...
	void AudioDecoder::run(std::stop_token theToken) {
		if (!this->haveWeBooted) {
			...
			if (avformat_open_input(*this->formatContext, "memory", nullptr, nullptr) < 0) {
				this->haveWeFailedBool.store(true);
				if (this->configManager->doWePrintFFMPEGErrorMessages()) {
					std::cout << DiscordCoreAPI::shiftToBrightRed() << "AudioDecoder::run() Error: Error opening AVFormatContext." << DiscordCoreAPI::reset() << std::endl << std::endl;
				}
				return;
			}
				...
				if (this->packet->stream_index == this->audioStreamIndex) {
					...
					if (returnValue >= 0) {
						...
						if (returnValue < 0 || newFrame->nb_samples == 0) {
							this->haveWeFailedBool.store(true);
							if (this->configManager->doWePrintFFMPEGErrorMessages()) {
								std::cout << DiscordCoreAPI::shiftToBrightRed() << "AudioDecoder::run() Error: Return value is less than zero!" << DiscordCoreAPI::reset() << std::endl << std::endl;
							}
							return;
						}
					} else {
