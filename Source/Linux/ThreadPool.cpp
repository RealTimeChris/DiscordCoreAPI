...
namespace DiscordCoreAPI {
    ...
    void ThreadPool::storeThread(std::string theKey, std::unique_ptr<CoRoutine<void>> thread) {
    ...
    ThreadPoolTimer ThreadPoolTimer::createPeriodicTimer(TimeElapsedHandlerTwo timeElapsedHandler, int64_t timeInterval) {
        ...
        threadPoolPtr->storeThread(threadPoolTimer.threadId, std::make_unique<CoRoutine<void>>(threadPoolTimer.run(timeInterval, timeElapsedHandler, true)));
    ...
    ThreadPoolTimer ThreadPoolTimer::createTimer(TimeElapsedHandlerTwo timeElapsedHandler, int64_t timeDelay) {
        ThreadPoolTimer threadPoolTimer{};
        auto threadPoolPtr = ThreadPoolTimer::threadsAtomic.load(std::memory_order_consume);
        threadPoolPtr->storeThread(threadPoolTimer.threadId, std::make_unique<CoRoutine<void>>(threadPoolTimer.run(timeDelay, timeElapsedHandler, false)));
        ThreadPoolTimer::threadsAtomic.store(threadPoolPtr, std::memory_order_release);
        return threadPoolTimer;
    }

    ThreadPoolTimer ThreadPoolTimer::createTimer(TimeElapsedHandler timeElapsedHandler, int64_t timeDelay) {
        ThreadPoolTimer threadPoolTimer{};
        auto threadPoolPtr = ThreadPoolTimer::threadsAtomic.load(std::memory_order_consume);
        threadPoolPtr->storeThread(threadPoolTimer.threadId, std::make_unique<CoRoutine<void>>(threadPoolTimer.run(timeDelay, timeElapsedHandler, false)));
        ThreadPoolTimer::threadsAtomic.store(threadPoolPtr, std::memory_order_release);
    ...
    ThreadPoolTimer::ThreadPoolTimer() {
        this->threadId = std::to_string(std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now().time_since_epoch()).count());
    }
