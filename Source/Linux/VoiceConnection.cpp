...
namespace DiscordCoreAPI {
	...
	EventDelegateToken VoiceConnection::onSongCompletion(EventDelegate<CoRoutine<void>, SongCompletionEventData> handler) {
		if (!this->areWeInstantiated) {
			this->areWeInstantiated = true;
			return onSongCompletionEvent.add(std::move(handler));
		}
		else {
	...
	void VoiceConnection::onSongCompletion(EventDelegateToken token) {
