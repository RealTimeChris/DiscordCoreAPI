...
namespace DiscordCoreInternal {
	...
	void BaseSocketAgent::sendMessage(std::string& dataToSend) {
		try {
			...
			if (!this->webSocket->writeData(newString)) {
	...
	void BaseSocketAgent::sendClosedMessage() {
		try {
			...
			if (!this->webSocket->writeData(theVectorNew)) {
				this->onClosedExternal();
			}
		}
		catch (...) {
	...
	DiscordCoreAPI::CoRoutine<void> BaseSocketAgent::run() {
		try {
			...
			this->connect();
			while (!this->doWeQuit) {
				if (this->doWeReconnect.load(std::memory_order_consume)) {
					this->onClosedInternal();
				}
				std::this_thread::sleep_for(std::chrono::milliseconds{ 1 });
				if (this->webSocket != nullptr) {
					this->webSocket->readData();
					this->handleBuffer();
				}
			}
			co_return;
		...
		try {
			...
			if (this->areWeCollectingData && payload.at("t") == "VOICE_SERVER_UPDATE" && !this->serverUpdateCollected) {
				if (this->serverUpdateCollected != true && this->stateUpdateCollected != true) {
					...
					this->voiceConnectionData.endPoint = payload.at("d").at("endpoint").get<std::string>();
					this->voiceConnectionData.token = payload.at("d").at("token").get<std::string>();
					this->voiceConnectionDataBuffer.send(this->voiceConnectionData);
			...
			if (payload.at("op") == 7) {
				...
				this->heartbeatTimer.reset(nullptr);
			...
			if (payload.at("op") == 9) {
				...
				srand(static_cast<uint32_t>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count()));
				this->areWeConnected.store(false, std::memory_order_release);
				this->currentReconnectTries += 1;
				...
				if (newVector.find("\r\n\r\n") != std::string::npos) {
					...
					if (headerOut.size()) {
						...
						if (status.size() >= 3 && status[1] == "101") {
							for (auto& theData : headerOut) {
								std::string::size_type sep = theData.find(": ");
								if (sep != std::string::npos) {
									std::string key = theData.substr(0, sep);
									std::string value = theData.substr(sep + 2, theData.length());
	...
	void BaseSocketAgent::onClosedExternal() {
		this->doWeReconnect.store(true, std::memory_order_release);
	}
	...
	void BaseSocketAgent::onClosedInternal() {
		...
		if (this->maxReconnectTries > this->currentReconnectTries) {
			...
			this->inputBuffer.clear();
			this->connect();
			this->doWeReconnect.store(false, std::memory_order_release);
		}
	...
	void VoiceSocketAgent::sendHeartBeat() {
		try {
			if (this->haveWeReceivedHeartbeatAck) {
				std::vector<uint8_t> heartbeatPayload = JSONIFY(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now().time_since_epoch()).count());
				if (this->webSocket != nullptr) {
	...
	void VoiceSocketAgent::connect() {
		try {
			this->authKey = DiscordCoreAPI::generateX64BaseEncodedKey();
			DiscordCoreAPI::waitForTimeToPass(this->baseSocketAgent->voiceConnectionDataBuffer, this->voiceConnectionData, 20000);
			this->baseUrl = this->voiceConnectionData.endPoint.substr(0, this->voiceConnectionData.endPoint.find(":"));
			this->relativePath = "/?v=4";
			this->heartbeatTimer.cancel();
			this->webSocket = std::make_unique<WebSocketSSLClient>(this->baseUrl, this->port, &this->inputBuffer00);
			this->state = WebSocketState::Initializing;
			std::string sendVector = "GET " + this->relativePath + " HTTP/1.1\r\nHost: " + this->baseUrl +
				"\r\nPragma: no-cache\r\nUser-Agent: DiscordCoreAPI/1.0\r\nUpgrade: WebSocket\r\nConnection: Upgrade\r\nSec-WebSocket-Key: " +
				this->authKey + "\r\nSec-WebSocket-Version: 13\r\n\r\n";
