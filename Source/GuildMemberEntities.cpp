...
namespace Jsonifier {
	...
	template<> struct Core<DiscordCoreAPI::AddGuildMemberData> {
		using OTy2 = DiscordCoreAPI::AddGuildMemberData;
		static constexpr auto parseValue = object("roles", &OTy2::roles, "access_token", &OTy2::accessToken, "guild_id", &OTy2::guildId, "user_id",
			&OTy2::userId, "nick", &OTy2::nick, "mute", &OTy2::mute, "deaf", &OTy2::deaf);
	};

	template<> struct Core<DiscordCoreAPI::ModifyCurrentGuildMemberData> {
		using OTy2 = DiscordCoreAPI::ModifyCurrentGuildMemberData;
		static constexpr auto parseValue = object("guild_id", &OTy2::guildId, "nick", &OTy2::nick, "reason", &OTy2::reason);
	};

	template<> struct Core<DiscordCoreAPI::ModifyGuildMemberData> {
		using OTy2 = DiscordCoreAPI::ModifyGuildMemberData;
		static constexpr auto parseValue =
			object("channel_id", &OTy2::currentChannelId, "deaf", &OTy2::deaf, "guild_id", &OTy2::guildId, "mute", &OTy2::mute, "nick", &OTy2::nick,
				"roles", &OTy2::roleIds, "user_id", &OTy2::guildMemberId, "voice_channel_id", &OTy2::newVoiceChannelId, "reason", &OTy2::reason);
	};
}
...
namespace DiscordCoreAPI {
	...
	CoRoutine<void> GuildMembers::removeGuildMemberAsync(RemoveGuildMemberData dataPackage) {
		...
		GuildMembers::httpsClient->submitWorkloadAndGetResult(workload);
		co_return;
	}
	...
	void GuildMembers::insertVoiceState(VoiceStateDataLight voiceState) {
		...
		std::cout << "INSERTING VIOCE STATE: " << voiceState.id << std::endl;
		std::cout << "INSERTING VIOCE STATE GUILD ID: " << voiceState.guildId << std::endl;
		std::cout << "INSERTING VIOCE STATE CHANNEL ID: " << voiceState.channelId << std::endl;
		if (Guilds::getCache().contains(voiceState.guildId)) {
