...
namespace DiscordCoreAPI {
	...
	std::basic_string_view<unsigned char> RTPPacketEncrypter::encryptPacket(const AudioFrameData& audioData) noexcept {
		if (this->keys.size() > 0) {
			this->sequence++;
	...
	void VoiceConnectionBridge::parseOutGoingVoiceData() noexcept {
		const std::basic_string_view<unsigned char> buffer = this->getInputBuffer();
		if (buffer.size() > 0) {
	...
	void VoiceConnection::sendVoiceData(const std::basic_string_view<unsigned char> responseData) noexcept {
		try {
	...
	void VoiceConnection::runBridge(std::stop_token token) noexcept {
		...
		int64_t timeTakesToProcessAndSleep{ 0 };
		int64_t iterationCount{ 0 };
		while (!token.stop_requested()) {
			iterationCount++;
			if (processAndSleepStopWatch.getTotalWaitTime() - (timeTakesToProcessAndSleep / iterationCount) >= 0) {
				std::this_thread::sleep_for(
					std::chrono::milliseconds{ processAndSleepStopWatch.getTotalWaitTime() - (timeTakesToProcessAndSleep / iterationCount) });
			}
			this->mixAudio();
			timeTakesToProcessAndSleep += processAndSleepStopWatch.totalTimePassed();
