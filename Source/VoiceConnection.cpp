...
namespace DiscordCoreAPI {
		...
		switch (static_cast<VoiceSocketOpCodes>(message.op)) {
			...
			case VoiceSocketOpCodes::Session_Description: {
				...
				if (arrayValue.didItSucceed) {
					std::basic_string<uint8_t> secretKey{};
	...
	void applyGainRamp(opus_int32* startSample, opus_int16* outputSample, int32_t numSamples,int32_t count, float currentGain, float endGain) noexcept {
	...
	void VoiceConnection::mixAudio() noexcept {
		...
		if (decodedSize > 0) {
			...
			applyGainRamp(this->upSampledVector.data(), this->downSampledVector.data(), decodedSize, voiceUserCount, this->previousGain,
				this->currentGain);
