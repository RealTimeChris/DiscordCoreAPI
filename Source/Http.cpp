...
namespace DiscordCoreInternal {
	...
	void HttpRnRBuilder::updateRateLimitData(
		std::unordered_map<std::string, std::string>& headersNew, RateLimitData* rateLimitData) {
		...
		if (headersNew.contains("x-ratelimit-reset-after")) {
			rateLimitData->msRemain =
				static_cast<int64_t>(ceil(stod(headersNew["x-ratelimit-reset-after"]))) * 1000;
		}
		if (headersNew.contains("x-ratelimit-remaining")) {
			rateLimitData->getsRemaining =
				static_cast<int64_t>(stoi(headersNew["x-ratelimit-remaining"])) * 1000;
	...
	HttpResponseData HttpRnRBuilder::finalizeReturnValues(RateLimitData* rateLimitDataPtr,
		HttpResponseData& theData) {
		if (theData.responseMessage.size() > 0) {
			if ((theData.responseMessage[0] == '{' &&
					theData.responseMessage[theData.responseMessage.size() - 1] == '}') ||
				(theData.responseMessage[0] == '[' &&
					theData.responseMessage[theData.responseMessage.size() - 1] == ']')) {
	...
	std::string HttpRnRBuilder::buildRequest(const HttpWorkloadData& workload) {
		...
		if (workload.baseUrl.find(".com") != std::string::npos) {
			baseUrlNew = workload.baseUrl.substr(workload.baseUrl.find("https://") +
					std::string("https://").size(),
				workload.baseUrl.find(".com") + std::string(".com").size() -
					std::string("https://").size());
		} else if (workload.baseUrl.find(".org") != std::string::npos) {
			baseUrlNew = workload.baseUrl.substr(workload.baseUrl.find("https://") +
					std::string("https://").size(),
				workload.baseUrl.find(".org") + std::string(".org").size() -
					std::string("https://").size());
		}
		std::string theReturnString{};
		if (workload.workloadClass == HttpWorkloadClass::Get ||
			workload.workloadClass == HttpWorkloadClass::Delete) {
			if (workload.workloadClass == HttpWorkloadClass::Get) {
				theReturnString +=
					"GET " + workload.baseUrl + workload.relativePath + " HTTP/1.1\r\n";
			} else if (workload.workloadClass == HttpWorkloadClass::Delete) {
				theReturnString +=
					"DELETE " + workload.baseUrl + workload.relativePath + " HTTP/1.1\r\n";
			}
			...
			if (workload.workloadClass == HttpWorkloadClass::Patch) {
				theReturnString +=
					"PATCH " + workload.baseUrl + workload.relativePath + " HTTP/1.1\r\n";
			} else if (workload.workloadClass == HttpWorkloadClass::Post) {
				theReturnString +=
					"POST " + workload.baseUrl + workload.relativePath + " HTTP/1.1\r\n";
			} else if (workload.workloadClass == HttpWorkloadClass::Put) {
				theReturnString =
					"PUT " + workload.baseUrl + workload.relativePath + " HTTP/1.1\r\n";
			}
			for (auto& [key, value]: workload.headersToInsert) {
				theReturnString += key + ": " + value + "\r\n";
			}
			...
			theReturnString += "Host: " + baseUrlNew + "\r\n";
			theReturnString +=
				"Content-Length: " + std::to_string(workload.content.size()) + "\r\n\r\n";
	...
	void HttpRnRBuilder::parseHeaders(std::string& other, HttpResponseData& theData) {
		try {
			if (other.find("\r\n\r\n") != std::string::npos) {
				...
				while (newString.size() > 0 && newString.find(":") != std::string::npos &&
					newString.find("\r\n") != std::string::npos) {
					...
					std::string value = lineString.substr(lineString.find(":") + 2,
						lineString.size() - key.size() - 2);
					value = value.substr(0, value.find("\r\n"));
					theData.responseHeaders[key] = value;
					std::string tempString{};
					tempString.insert(tempString.begin(), newString.begin() + currentOffset,
						newString.end());
					newString = tempString;
				}
				if (theData.responseHeaders.contains("Transfer-Encoding") &&
					theData.responseHeaders["Transfer-Encoding"] == "chunked") {
			...
			if (this->isItChunked) {
				...
				if (other.find("\r\n0\r\n\r\n") != std::string::npos) {
					while (other.find("\r\n") != other.find("\r\n0\r\n\r\n")) {
						...
						this->clearCRLF(other);
						if (other.find("\r\n") != other.find("\r\n0\r\n\r\n")) {
							theData.responseMessage.insert(theData.responseMessage.end(),
								other.begin(), other.begin() + other.find("\r\n"));
							other.erase(other.begin(), other.begin() + other.find("\r\n") + 2);
						}
					}
					theData.responseMessage.insert(theData.responseMessage.end(), other.begin(),
						other.begin() + other.find("\r\n0\r\n\r\n"));
					return false;
				} else {
					return true;
				...
				if (other.size() >= static_cast<size_t>(theData.contentSize)) {
					theData.responseMessage.insert(theData.responseMessage.end(), other.begin(),
						other.begin() + theData.contentSize);
					return false;
				} else {
					return true;
				}
	...
	void HttpRnRBuilder::parseCode(std::string& other, HttpResponseData& theData) {
		if (other.find("HTTP/1.") != std::string::npos) {
			...
			for (size_t x = other.find("HTTP/1.") + std::string("HTTP/1.").size() + 1;
				 x < other.size(); x += 1) {
			...
			theData.responseCode =
				stoll(other.substr(firstNumberIndex, lastNumberIndex - firstNumberIndex));
	...
	HttpConnection* HttpConnectionManager::getConnection() {
		...
		if (!Globals::httpConnections.contains(std::this_thread::get_id())) {
			Globals::httpConnections[std::this_thread::get_id()] =
	...
	void HttpConnectionManager::initialize() {
		for (int64_t enumOne = static_cast<int64_t>(HttpWorkloadType::Unset);
			 enumOne != static_cast<int64_t>(HttpWorkloadType::LAST); enumOne++) {
			std::unique_ptr<RateLimitData> rateLimitData{ std::make_unique<RateLimitData>() };
			rateLimitData->tempBucket =
				std::to_string(std::chrono::duration_cast<std::chrono::nanoseconds>(
					std::chrono::system_clock::now().time_since_epoch())
								   .count());
			Globals::rateLimitValueBuckets[static_cast<HttpWorkloadType>(enumOne)] =
				rateLimitData->tempBucket;
	...
	HttpClient::HttpClient(const std::string& botTokenNew, bool doWePrintHttpSuccessMessagesNew,
		bool doWePrintHttpErrorMessagesNew, bool doWePrintFFMPEGSuccessMessagesNew,
		bool doWePrintFFMPEGErrorMessagesNew, bool doWePrintWebSocketErrorMessagesNew)
		: botToken(botTokenNew), doWePrintFFMPEGErrorMessages(doWePrintFFMPEGErrorMessagesNew),
		  doWePrintHttpSuccessMessages(doWePrintHttpSuccessMessagesNew),
		  doWePrintHttpErrorMessages(doWePrintHttpErrorMessagesNew),
		  doWePrintFFMPEGSuccessMessages(doWePrintFFMPEGSuccessMessagesNew),
	...
	HttpResponseData HttpClient::executeByRateLimitData(const HttpWorkloadData& workload,
		...
		RateLimitData* rateLimitDataPtr =
			Globals::rateLimitValues[Globals::rateLimitValueBuckets[workload.workloadType]].get();
		...
		} else if (workload.workloadType == HttpWorkloadType::Delete_Message ||
		...
		if (rateLimitDataPtr->areWeASpecialBucket) {
			rateLimitDataPtr->msRemain =
				static_cast<int64_t>(ceil(static_cast<float>(5000) / static_cast<float>(4)));
		...
		if (timeRemaining > 0) {
			if (this->doWePrintHttpSuccessMessages) {
				std::cout << DiscordCoreAPI::shiftToBrightBlue()
						  << "We're waiting on rate-limit: " << timeRemaining
			...
			while (targetTime > currentTime) {
				...
				if (timeRemaining <= 20) {
					...
					std::this_thread::sleep_for(std::chrono::milliseconds{
						static_cast<int64_t>(ceil(timeRemaining * 80.0f / 100.0f)) });
				}
			}
		}
		...
		if (!Globals::rateLimitValues.contains(rateLimitDataPtr->bucket)) {
			std::unique_ptr<RateLimitData> rateLimitData{ std::make_unique<RateLimitData>() };
			Globals::rateLimitValues[Globals::rateLimitValueBuckets[workload.workloadType]].swap(
				rateLimitData);
		...
		if (returnData.responseCode == 204 || returnData.responseCode == 201 ||
			returnData.responseCode == 200) {
			if (this->doWePrintHttpSuccessMessages) {
				std::cout << DiscordCoreAPI::shiftToBrightGreen()
						  << workload.callStack + " Success: " << returnData.responseCode << ", "
						  << returnData.responseMessage << DiscordCoreAPI::reset() << std::endl
						  << std::endl;
			}
		} else {
			if (returnData.responseCode == 429) {
				rateLimitDataPtr->msRemain =
					static_cast<int64_t>(
						ceil(nlohmann::json::parse(returnData.responseMessage)["retry_after"]
								 .get<double>())) *
					1000;
				...
				if (this->doWePrintHttpErrorMessages) {
					std::cout << DiscordCoreAPI::shiftToBrightRed()
							  << workload.callStack +
							"::httpRequest(), We've hit rate limit! Time Remaining: "
							  << std::to_string(
									 Globals::rateLimitValues
										 [Globals::rateLimitValueBuckets[workload.workloadType]]
											 ->msRemain)
							  << DiscordCoreAPI::reset() << std::endl
							  << std::endl;
				}
				...
				std::cout << DiscordCoreAPI::shiftToBrightRed()
						  << workload.callStack + " Error: Code = "
						  << std::to_string(returnData.responseCode) +
						", Message = " + returnData.responseMessage
						  << DiscordCoreAPI::reset() << std::endl
						  << std::endl;
			}
		}
	...
	HttpResponseData HttpClient::httpRequestInternal(const HttpWorkloadData& workload,
		HttpConnection& theConnection, RateLimitData* rateLimitDataPtr) {
		try {
			theConnection.resetValues();
			int64_t currentTimeDistance = std::chrono::duration_cast<std::chrono::milliseconds>(
											  std::chrono::system_clock::now().time_since_epoch())
											  .count() -
			...
			if (theConnection.doWeConnect ||
				(theConnection.lastTimeUsed != 0 && currentTimeDistance >= 30000)) {
				theConnection.connect(workload.baseUrl);
				theConnection.doWeConnect = false;
			}
			theConnection.lastTimeUsed = std::chrono::duration_cast<std::chrono::milliseconds>(
				std::chrono::system_clock::now().time_since_epoch())
											 .count();
			...
			if (this->doWePrintHttpErrorMessages) {
				DiscordCoreAPI::reportException(
					workload.callStack + "::HttpClient::executeHttpRequest()");
			}
			return this->httpRequestInternal(workload, theConnection, rateLimitDataPtr);
		}
	}

	HttpResponseData HttpClient::getResponse(HttpConnection& theConnection,
		RateLimitData* rateLimitDataPtr) {
		...
		while (true) {
			...
			if (theConnection.checkForHeadersToParse(theConnection.getInputBuffer()) &&
				!theConnection.doWeHaveHeaders && !stopWatch.hasTimePassed()) {
				theConnection.parseHeaders(theConnection.getInputBuffer(), theData);
			}
			if (static_cast<int64_t>(theConnection.getInputBuffer().size()) >=
					theData.contentSize &&
				!theConnection.parseChunk(theConnection.getInputBuffer(), theData)) {
				break;
			}
	...
	std::vector<HttpResponseData> HttpClient::httpRequest(
		const std::vector<HttpWorkloadData>& workload) {
	...
	HttpResponseData HttpClient::httpRequest(HttpWorkloadData& workload) {
		...
		while (HttpWorkloadData::workloadIdsInternal[workload.workloadType].load() <
				workload.thisWorkerId.load() &&
	...
	template<>
	std::vector<HttpResponseData> HttpClient::submitWorkloadAndGetResult(
		const std::vector<HttpWorkloadData>& workloadNew) {
		std::vector<HttpWorkloadData> workload = workloadNew;
		return this->httpRequest(workload);
	}

	template<>
	HttpResponseData HttpClient::submitWorkloadAndGetResult(HttpWorkloadData& workloadNew) {
		HttpWorkloadData workload = workloadNew;
		workload.headersToInsert["Authorization"] = "Bot " + this->botToken;
		workload.headersToInsert["User-Agent"] = "DiscordBot (https://discordcoreapi.com/ 1.0)";
		if (workload.payloadType == PayloadType::Application_Json) {
