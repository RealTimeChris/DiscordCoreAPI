...
namespace DiscordCoreInternal {
	...
	HttpsConnection::HttpsConnection() noexcept : HttpsRnRBuilder(){};
	...
	std::unordered_map<std::string, DiscordCoreAPI::UniquePtr<RateLimitData>>& RateLimitManager::getRateLimitValues() noexcept {
		return rateLimitValues;
	}

	std::unordered_map<HttpsWorkloadType, std::string>& RateLimitManager::getRateLimitValueBuckets() noexcept {
	...
	HttpsConnection& RateLimitManager::getConnection(HttpsWorkloadType workloadType) noexcept {
		...
		if (!httpsConnections.contains(workloadType)) {
			httpsConnections[workloadType] = DiscordCoreAPI::makeUnique<HttpsConnection>();
		}
	...
	void RateLimitManager::initialize() noexcept {
		for (int64_t enumOne = static_cast<int64_t>(HttpsWorkloadType::Unset); enumOne != static_cast<int64_t>(HttpsWorkloadType::LAST); enumOne++) {
			DiscordCoreAPI::UniquePtr<RateLimitData> rateLimitData{ DiscordCoreAPI::makeUnique<RateLimitData>() };
	...
	HttpsResponseData HttpsClient::executeByRateLimitData(HttpsConnection& connection, RateLimitData& rateLimitData) noexcept {
		...
		if (timeRemaining.count() > 0) {
			Globals::MessagePrinter::printSuccess<Globals::MessageType::Https>(
				"We're waiting on rate-limit: " + std::to_string(timeRemaining.count()));
		...
		if (!connectionManager.getRateLimitValues().contains(rateLimitData.bucket)) {
			DiscordCoreAPI::UniquePtr<RateLimitData> rateLimitData{ DiscordCoreAPI::makeUnique<RateLimitData>() };
			connectionManager.getRateLimitValues()[connectionManager.getRateLimitValueBuckets()[connection.workload.workloadType]].swap(
				rateLimitData);
			connectionManager.getRateLimitValueBuckets()[connection.workload.workloadType] = currentBucket;
		...
		if (returnData.responseCode == 204 || returnData.responseCode == 201 || returnData.responseCode == 200) {
			...
			rateLimitData.sampledTimeInMs.store(std::chrono::duration_cast<Milliseconds>(HRClock::now().time_since_epoch()));
			Globals::MessagePrinter::printError<Globals::MessageType::Https>(connection.workload.callStack +
