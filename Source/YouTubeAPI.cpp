...
namespace DiscordCoreAPI {
	...
	std::vector<Song> YouTubeRequestBuilder::collectSearchResults(std::string searchQuery) {
		...
		if (returnData[0].responseCode != 200) {
			std::cout << shiftToBrightRed() << "YouTubeRequestBuilder::collectSearchResults Error: " << returnData[0].responseCode << returnData[0].responseMessage.c_str() << reset() << std::endl;
		}
		...
		for (uint32_t x = 0; x < responseData.size(); x += 1) {
			...
			if (responseData[x].responseCode != 204 && responseData[x].responseCode != 201 && responseData[x].responseCode != 200) {
				std::cout << shiftToBrightRed() << "Song::collectDownloadInfo() 01 Error: " << responseData[x].responseCode << ", " << responseData[x].responseMessage << std::endl << reset() << std::endl;
			}
	...
	Song YouTubeRequestBuilder::constructDownloadInfo(GuildMemberData guildMember, Song newSong) {
		...
		if (resultHTMLBody[0].responseCode != 204 && resultHTMLBody[0].responseCode != 201 && resultHTMLBody[0].responseCode != 200) {
			std::cout << shiftToBrightRed() << "Song::collectDownloadInfo() 01 Error: " << resultHTMLBody[0].responseCode << ", " << resultHTMLBody[0].responseMessage << std::endl << reset() << std::endl;
		}
		...
		if (responseMessage02[0].responseCode != 204 && responseMessage02[0].responseCode != 201 && responseMessage02[0].responseCode != 200) {
			std::cout << shiftToBrightRed() << "Song::collectDownloadInfo() 02 Error: " << responseMessage02[0].responseCode << ", " << responseMessage02[0].responseMessage << std::endl << reset() << std::endl;
		}
