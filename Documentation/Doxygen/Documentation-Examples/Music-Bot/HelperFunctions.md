...
#pragma once
	...
	Bool areWeInADM(InputEventData eventData, Channel channel, Bool displayResponse = true) {
		...
		if (currentChannelType == ChannelType::Dm) {
			if (displayResponse) {
				...
				UniquePtr<EmbedData> msgEmbed(new EmbedData());
				...
				UniquePtr<RespondToInputEventData> responseData(new RespondToInputEventData(eventData));
	...
	Bool checkIfAllowedPlayingInChannel(InputEventData eventData, DiscordGuild discordGuild) {
		...
		if (discordGuild.data.musicChannelIds.size() > 0) {
			...
			if (isItFound == false) {
				...
				UniquePtr<RespondToInputEventData> replyMessageData(new RespondToInputEventData(eventData));
	...
	Bool checkIfWeHaveControl(InputEventData eventData, DiscordGuild guildData, GuildMember guildMember) {
		...
		if (!doWeHaveControl) {
			...
			UniquePtr<RespondToInputEventData> dataPackage(new RespondToInputEventData(eventData));
