...
namespace DiscordCoreAPI {
    ...
    public:
        ...
        SongDecoder(BuildSongDecoderData dataPackage, shared_ptr<DiscordCoreInternal::ThreadContext> threadContextNew) : agent(*threadContextNew->scheduler) {
        ...
        void run() {
            if (!this->haveWeBooted) {
                ...
                if (this->ioContext == nullptr) {
                    cout << "Failed to allocate ioContext.\n\n";
                }
                ...
                if (avformat_open_input(&formatContext, "memory", nullptr, nullptr) < 0) {
                    cout << "Error opening format context.\n\n";
                    return;
                }
                ...
                if (ret < 0) {
                    ...
                    if (!audioStream) {
                        cout << "Could not find collect an audio stream.\n\n";
                        done();
                        return;
                    }
    ...
    protected:
        int audioStreamIndex{ 0 }, audioFrameCount{ 0 }, totalFileSize{ 0 }, bufferMaxSize{ 0 }, bytesRead{ 0 };
        ...
        static int FileStreamRead(void* opaque, uint8_t* buf, int) {   
            ...
            while (!try_receive(stream->dataBuffer, stream->currentBuffer)) {};
